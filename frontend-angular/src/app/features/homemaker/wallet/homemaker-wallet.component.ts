    ...
    <div class="wallet-container">
      ...
      <div class="wallet-summary" *ngIf="wallet">
        <div class="balance-card">
          ...
          <p class="amount">₹{{ wallet.balance?.toFixed(2) || '0.00' }}</p>
          <p class="hint">Available to withdraw</p>
        </div>
        <div class="stats-card">
          <div class="stat">
            <span class="label">Total Earnings</span>
            <span class="value">₹{{ wallet.totalEarnings?.toFixed(2) || '0.00' }}</span>
          </div>
          <div class="stat">
            <span class="label">Pending Amount</span>
            <span class="value">₹{{ wallet.pendingAmount?.toFixed(2) || '0.00' }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="withdraw-section" *ngIf="wallet && bankAccounts.length > 0">
        ...
        <div class="withdraw-form">
          ...
          <p class="note">Minimum withdrawal ₹100. Money will be credited to your primary bank account.</p>
        </div>
      </div>
...
export class HomemakerWalletComponent implements OnInit {
  wallet: any = null;
  ...
  loadWallet() {
    ...
    this.homemakerService.getWallet(this.homemakerId).subscribe({
      ...
      error: () => { this.wallet = { balance: 0, totalEarnings: 0, pendingAmount: 0 }; this.loading = false; }
    });
  }

  loadBankAccounts() {
    this.homemakerService.getBankAccounts(this.homemakerId).subscribe({
  ...
  addBankAccount() {
    ...
    this.bankError = '';
    this.homemakerService.addBankAccount(this.homemakerId, this.newBank).subscribe({
      next: () => {
        this.loadBankAccounts();
        this.cancelAddBank();
      },
  ...
  deleteBankAccount(accountId: number) {
    if (confirm('Are you sure you want to delete this bank account?')) {
      this.homemakerService.deleteBankAccount(this.homemakerId, accountId).subscribe({
  ...
  canWithdraw(): boolean {
    ...
    return this.withdrawAmount >= 100 && this.withdrawAmount <= this.wallet.balance;
  }

  withdraw() {
    if (!this.canWithdraw()) {
      this.error = 'Enter an amount between ₹100 and your available balance.';
      return;
    }
    ...
    this.homemakerService.requestWithdrawal(this.homemakerId, { amount: this.withdrawAmount }).subscribe({
      next: (response: any) => {
        this.loading = false;
        this.withdrawAmount = null;
        this.success = response.message || 'Withdrawal request submitted successfully!';
        this.loadWallet();
      },
      error: (err) => {
        this.loading = false;
